...
class TestClaudeGeneration:
    ...
    def test_deny_read_generates_all_file_tools(
        self, gen: ClaudeGenerator, config: AppConfig
    ) -> None:
        """FR-006: denyRead generates deny for ALL file tools."""
        rules = [
    ...
    def test_bash_deny_generates_bash_deny(
        ...
        """FR-010: Bash deny → Bash(cmd:*) in deny."""
        rules = [
    ...
    def test_deny_read_and_deny_write_overlap(
        ...
        # From both denyRead and denyWrite
        assert "Write(**/.env/**)" in deny or "Write(**/.env)" in deny
        assert "Edit(**/.env/**)" in deny or "Edit(**/.env)" in deny
        assert "MultiEdit(**/.env/**)" in deny or "MultiEdit(**/.env)" in deny
