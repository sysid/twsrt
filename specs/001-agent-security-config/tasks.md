...
- [X] T012 Write tests for CLI init and version commands in tests/bin/test_cli.py: init creates ~/.config/twsrt/ dir + config.toml + bash-rules.json, init with existing files skips with warning, init --force overwrites, bare invocation shows help, -V prints version, `twsrt version` prints version
