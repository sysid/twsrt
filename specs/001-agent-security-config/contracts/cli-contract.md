...
| `--config` | `-c` | Path | `~/.twsrt/config.toml` | Config file path |
...
Initialize the `~/.twsrt/` config directory with default files.
...
**Behavior**:
- Creates `~/.twsrt/` directory
- Creates `~/.twsrt/config.toml` with default paths
- Creates `~/.twsrt/bash-rules.json` with empty deny/ask arrays
...
When `~/.twsrt/config.toml` does not exist, twsrt uses these defaults:
...
| Config dir missing | `Error: Config directory ~/.twsrt/ not found. Run 'twsrt init' first.` | 1 |
