...
`~/.config/twsrt/bash-rules.json` for Bash deny/ask rules — and generates
...
**Storage**: JSON files (read: `~/.srt-settings.json`, `~/.config/twsrt/bash-rules.json`; write: `~/.claude/settings.json`), TOML config (`~/.config/twsrt/config.toml`)
...
src/
  twsrt/
    ...
    lib/
      __init__.py
      config.py          # TOML config loading (~/.config/twsrt/config.toml)
