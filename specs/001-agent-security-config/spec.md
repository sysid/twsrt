...
| Bash rules (`~/.config/twsrt/bash-rules.json`) | Agent-level (tool permissions) | Bash command deny/ask | SRT cannot distinguish `bash rm` from `bash git push` |
...
~/.config/twsrt/bash-rules.json                sandbox.network     — FULLY replaced
...
`~/.config/twsrt/bash-rules.json`. He runs twsrt to generate the Claude
...
`~/.config/twsrt/bash-rules.json` with deny/ask entries. Verify the
...
`~/.config/twsrt/bash-rules.json`. Verify the output produces valid
Copilot CLI flags.

**Acceptance Scenarios**:

1. **Given** `~/.config/twsrt/bash-rules.json` with
   `deny: ["rm", "sudo", "git push --force"]`,
...
3. **Given** `~/.config/twsrt/bash-rules.json` with
...
- What happens when `~/.config/twsrt/` directory does not exist?
...
- **FR-002**: System MUST read Bash tool deny/ask rules from
  `~/.config/twsrt/bash-rules.json` as the canonical source for
  agent-level Bash command permissions.

- **FR-003**: System MUST use `~/.config/twsrt/` as its configuration
...
- **Canonical Source**: A file that authoritatively defines
  ...
  `~/.config/twsrt/bash-rules.json` for Bash tool permissions.
...
- **Config Directory** (`~/.config/twsrt/`): The directory containing
...
- `~/.config/twsrt/bash-rules.json` is a canonical INPUT, not a
...
- The existing permission rules in `settings.json` and
  ...
  from `settings.json` into `~/.config/twsrt/bash-rules.json`.
