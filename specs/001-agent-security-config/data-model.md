...
BASH_RULES      — from ~/.twsrt/bash-rules.json
```
...
Application configuration loaded from `~/.twsrt/config.toml`.

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| srt_path | `Path` | `~/.srt-settings.json` | Path to SRT settings |
| bash_rules_path | `Path` | `~/.twsrt/bash-rules.json` | Path to Bash rules |
...
| (EXECUTE, DENY, cmd) | `Bash({cmd}:*)` in `permissions.deny` |
